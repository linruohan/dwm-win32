...
int luaopen_dwm_client(lua_State *L) {
  const luaL_Reg lib[] = {{"clients", f_clients},
                          {"client", f_client},
                          {"show", f_show},
                          {"hide", f_hide},
                          {"border", f_border},
...
int f_border(lua_State *L) {
  ...
  if (border > 0)
    SetWindowLong(hwnd, GWL_STYLE,
                  (GetWindowLong(hwnd, GWL_STYLE) | (WS_CAPTION | WS_SIZEBOX)));
  else {
    SetWindowLong(
        hwnd, GWL_STYLE,
        (GetWindowLong(hwnd, GWL_STYLE) & ~(WS_CAPTION | WS_SIZEBOX)) |
