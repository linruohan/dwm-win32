...
static void drawtext(const wchar_t *text, unsigned long col[ColLast],
                     bool invert);
...
static int bx, by, bh,
    blw; /* bar geometry x, y, height and layout symbol width */
static int wx, wy, ww,
    wh; /* window area geometry x, y, width, height, bar excluded */
...
/* elements of the window whose color should be set to the values in the array
 * below */
...
void applyrules(Client *c) {
  ...
  for (i = 0; i < LENGTH(rules); i++) {
    ...
    if ((!r->title || wcsstr(getclienttitle(c->hwnd), r->title)) &&
        ...
        (!r->processname ||
         wcsstr(getclientprocessname(c->hwnd), r->processname))) {
...
void clearurgent(Client *c) { c->isurgent = false; }
...
void drawbar(void) {
  ...
  if (showclock) {
    ...
    if (showutcclock) {
      ...
      swprintf(timestr, sizeof(timestr), L"%ls | UTC: %ls", localtimestr,
               utctimestr);
    } else {
...
void drawtext(const wchar_t *text, unsigned long col[ColLast], bool invert) {
  RECT r = {
      .left = dc.x, .top = dc.y, .right = dc.x + dc.w, .bottom = dc.y + dc.h};
...
void eprint(bool premortem, const wchar_t *errstr, ...) {
  ...
  if (vswprintf(buffer + wcslen(program_name), num_of_chars + 1, errstr, ap) <
      0) {
  ...
  if (premortem)
    MessageBoxW(NULL, buffer, L"dwm-win32 has encountered an error",
...
void focus(Client *c) {
  ...
  if (sel) {
    ...
    // (better performance)
    // bert1337/dwm-win32/commit/6e331064f5db68ff3b7a2b2a38cdd3810e94d38a
...
Client *managechildwindows(Client *p) {
  Client *c, *t;
...
BOOL isclientdebugged(HWND hwnd) {
...
bool iscloaked(HWND hwnd) {
  int cloaked_val;
  HRESULT h_res = DwmGetWindowAttribute(hwnd, DWMWA_CLOAKED, &cloaked_val,
                                        sizeof(cloaked_val));
...
bool ismanageable(HWND hwnd) {
  ...
  if (wcsstr(classname, L"Windows.UI.Core.CoreWindow") ||
      (wcsstr(title, L"Windows Shell Experience Host") ||
       ...
       wcsstr(title, L"Action center") || wcsstr(title, L"New Notification") ||
       wcsstr(title, L"Date and Time Information") ||
       wcsstr(title, L"Volume Control") ||
       wcsstr(title, L"Network Connections") || wcsstr(title, L"Cortana") ||
  ...
  if (wcsstr(classname, L"ForegroundStaging") ||
      ...
      wcsstr(classname, L"Static") || wcsstr(classname, L"Scrollbar") ||
...
void quit(const Arg *arg) { PostMessage(dwmhwnd, WM_CLOSE, 0, 0); }
...
void resize(Client *c, int x, int y, int w, int h) {
  ...
  if (c->x != x || c->y != y || c->w != w || c->h != h) {
    ...
    SetWindowPos(c->hwnd, HWND_TOP, c->x - 7, c->y, c->w + 14, c->h + 7,
                 SWP_NOACTIVATE);
  }
}
...
void CALLBACK wineventproc(HWINEVENTHOOK heventhook, DWORD event, HWND hwnd,
                           LONG object, LONG child, DWORD eventthread,
                           DWORD eventtime_ms) {
...
void setborder(Client *c, bool border) {
  if (!c->ignoreborder) {
    if (border) {
      ...
      /* XXX: ideally i would like to use the standard window border facilities
       * and just modify the color with SetSysColor but this only seems to work
       * if we leave WS_SIZEBOX enabled which is not optimal.
       */
      SetWindowLong(
          c->hwnd, GWL_STYLE,
          (GetWindowLong(c->hwnd, GWL_STYLE) & ~(WS_CAPTION | WS_SIZEBOX)) |
...
void setvisibility(HWND hwnd, bool visibility) {
  SetWindowPos(hwnd, 0, 0, 0, 0, 0,
               (visibility ? SWP_SHOWWINDOW : SWP_HIDEWINDOW) | SWP_NOACTIVATE |
...
void load_user_script(lua_State *L) {
  (void)luaL_dostring(
      L, "local dwm = require 'dwm'\n"
         "local dwm_config_dir = (os.getenv('XDG_CONFIG_HOME') or "
         "os.getenv('HOME') or os.getenv('USERPROFILE')) .. '/.config/dwm'\n"
...
void setup(lua_State *L, HINSTANCE hInstance) {
  ...
  dwmhwnd = CreateWindowExW(0, NAME, NAME, 0, 0, 0, 0, 0, HWND_MESSAGE, NULL,
                            hInstance, NULL);
  ...
  wineventhook =
      SetWinEventHook(EVENT_OBJECT_CLOAKED, EVENT_OBJECT_UNCLOAKED, NULL,
...
void showclientinfo(const Arg *arg) {
  ...
  swprintf(buffer, sizeof(buffer),
           L"ClassName:  %s\nTitle:  %s\nProcessName: %s",
           getclientclassname(hwnd), getclienttitle(hwnd),
           getclientprocessname(hwnd));
...
void spawn(const Arg *arg) {
  ShellExecuteW(NULL, NULL, ((wchar_t **)arg->v)[0], ((wchar_t **)arg->v)[1],
                NULL, SW_SHOWDEFAULT);
}
...
void tile(void) {
  ...
  for (i = 0, c = nexttiled(c->next); c; c = nexttiled(c->next), i++) {
    resize(c, x, y, w - 2 * c->bw, /* remainder */
           ((i + 1 == n) ? wy + wh - y - 2 * c->bw : h - 2 * c->bw));
    if (h != wh)
      y = c->y + HEIGHT(c);
  }
}
...
void updatebar(void) {
  SetWindowPos(barhwnd, showbar ? HWND_TOPMOST : HWND_NOTOPMOST, bx, by, ww, bh,
               (showbar ? SWP_SHOWWINDOW : SWP_HIDEWINDOW) | SWP_NOACTIVATE |
                   SWP_NOSENDCHANGING);
}
...
void zoom(const Arg *arg) {
  ...
  if (!lt[sellt]->arrange || lt[sellt]->arrange == monocle ||
      (sel && sel->isfloating))
    return;
...
void forcearrange(const Arg *arg) {
  Client *c;
  for (c = clients; c; c = c->next) {
    c->x = 0;
    c->y = 0;
...
LRESULT __stdcall HookCallback(int nCode, WPARAM wParam, LPARAM lParam) {
  ...
  if (nCode >= 0) {
    ...
    if (wParam == WM_KEYDOWN) {
       ...
       * - Dentro il for fare l'if che c'è qui sotto lanciando anche la
       *funzione
       * - PROFIT ???
       */
      int i;
      ...
      for (i = 0; i < LENGTH(keys); i++) {
        ...
        if (kbdStruct.vkCode == (int)keys[i].key) {
          int k = 0;
          int a = ((keys[i].mod & MOD_ALT) & (1 << k)) >>
                  k; // Controllo che MOD_ALT Sia hotkey
          k = 1;
          int c = ((keys[i].mod & MOD_CONTROL) & (1 << k)) >>
                  k; // Controllo che MOD_CONTROL Sia hotkey
          k = 2;
          int s = ((keys[i].mod & MOD_SHIFT) & (1 << k)) >>
                  k; // Controllo che MOD_SHIFT Sia hotkey
          k = 3;
          int w = ((keys[i].mod & MOD_WIN) & (1 << k)) >>
                  k; // Controllo che MOD_WIN Sia hotkey

          if ((w == win) && (a == alt) && (c == ctrl) && (s == shift)) {
...
void SetHook() {
  ...
  // information about it at MSDN. The last 2 parameters are NULL, 0 because the
  // callback function is in the same thread and window as the function that
  // sets and releases the hook. If you create a hack you will not need the
  // callback function in another place then your own code file anyway. Read
  // more about it at MSDN.
...
// catches unhandled exceptions and performs necessary cleanup before a crash,
// you can use Structured Exception Handling (SEH) in Windows
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  // the __try block contains the main program code. If an unhandled exception
  // occurs anywhere in this block, control immediately transfers to the
  // __except block. The cleanupOnException function is then called to unmanage
  // all windows before the program crashes. Establish an exception handler
  __try {
    setup(L, hInstance);
